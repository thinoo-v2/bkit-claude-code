...
// ============================================================
// v1.4.4 FR-11: Level별 Phase Skip 자동화
// ============================================================

/**
 * Level별로 건너뛸 수 있는 Phase 맵
 * - Starter: Phase 4,5,7,8 건너뛰기 가능 (정적 웹)
 * - Dynamic: Phase 8만 건너뛰기 가능 (fullstack but simpler review)
...
const LEVEL_PHASE_MAP = {
  Starter: {
    ...
    skipReasons: {
      4: '정적 웹사이트는 API 설계가 필요 없음',
      5: '단순 프로젝트는 디자인 시스템 생략 가능',
      7: '기본 SEO/보안만 적용',
      8: '간단한 프로젝트는 리뷰 생략 가능'
    }
  },
  Dynamic: {
    ...
    optional: [8],
    skipReasons: {
      8: '중소규모 fullstack 프로젝트는 리뷰 단계 선택적'
    }
  },
  Enterprise: {
...
function getLevelPhaseGuide(level) {
  ...
  const optionalStr = levelConfig.optional.length > 0
    ? `\n선택적: ${levelConfig.optional.map(p => `Phase ${p}: ${phaseNames[p]}`).join(', ')}`
    : '';
